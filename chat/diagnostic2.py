...
def calculate_loss(model, tokenizer, convo_history, bot1_diag_response, ground_truth_answers):
    ...
    cat_ground_truth_answers =  torch.concat([inputs, ground_truth_answers], dim=-1).to(device) # add tensors together
    ...
    hiddens_cat_ground_truth_answers = outputs.hidden_states[-1][:, -1+(-1*ground_truth_answers.shape[-1]):-1]
    print("hiddens_diag_response zie: ", hiddens_diag_response.size(), "\n")
    print("hiddens_cat_diag_response zie: ", hiddens_cat_ground_truth_answers.size(), "\n")

    # calculate loss
    logits  = model.lm_head(hiddens_diag_response) #compare model output against actual tokens
    logits_truth = model.lm_head(hiddens_cat_ground_truth_answers)
    max_length = max(logits.shape[1], logits_truth.shape[1])
    if logits.shape[1] < max_length:
        # Padding to the right (at the end of each sequence)
        logits = torch.nn.functional.pad(logits, (0, 0, 0, max_length - logits.shape[1]))
    elif logits_truth.shape[1] < max_length:
        logits_truth = torch.nn.functional.pad(logits_truth, (0, 0, 0, max_length - logits_truth.shape[1]))
    ...
    loss = loss_fct(logits.view(-1, logits.size(-1)), logits_truth.view(-1, logits.size(-1)))
